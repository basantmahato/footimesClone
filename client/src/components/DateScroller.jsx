...
const DataScroller = ({ matchDates = [], selectedDate, onSelectDate }) => {
  ...
  // const parsedDates = matchDates.map((d) => new Date(d));
  const parsedDates = matchDates.map((d) => {
    const [year, month, day] = d.split("-");
    return new Date(year, month - 1, day); // local timezone date
  });
  ...
  return (
    <div className="flex items-center fixed top-13 left-0 right-0 z-9 bg-[#0f0f0f] p-2 text-white overflow-hidden">
          ...
          return (
            <button
              ...
              className={`
                ...
                border border-dashed border-pink-500 transition-all duration-300
                ${
                  active
