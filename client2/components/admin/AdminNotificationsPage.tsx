...
import { Bell, Trash2, ToggleLeft, ToggleRight, Plus, Send, Edit3, X } from 'lucide-react';
...
const AdminNotificationsPage = () => {
  ...
  const handleEdit = (n: Notification) => {
    setEditId(n._id);
    setForm({ message: n.message, type: n.type });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        const res = await axios.patch(`https://api.footimes.com/api/notifications/${editId}`, form);
        setNotifications(notifications.map(n => n._id === editId ? res.data : n));
      ...
      toast.error(editId ? "Failed to update." : "Failed to push notification.");
    }
  };
          ...
          <form onSubmit={handleSubmit} className={`${themeCard} border rounded-2xl p-6 sticky top-6`}>
            <h3 className="text-sm font-bold mb-6 flex items-center justify-between">
              <span className="flex items-center gap-2">
                {editId ? <Edit3 size={16} className="text-pink-500" /> : <Plus size={16} className="text-pink-500" />}
                {editId ? "Edit Notification" : "Push New Alert"}
              </span>
              {editId && (
                <button 
                  type="button" 
            ...
            <div className="space-y-4">
              ...
              >
                {editId ? <Edit3 size={14} /> : <Send size={14} />}
                {editId ? "Update Alert" : "Push Live"}
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="space-y-4">
            {loading ? (
              ...
              notifications.map((n) => (
                <div key={n._id} className={`${themeCard} border rounded-2xl p-5 flex items-center justify-between gap-4 transition-all hover:shadow-lg ${editId === n._id ? 'ring-1 ring-pink-500' : ''}`}>
                  ...
                  <div className="flex items-center gap-1">
                    <button 
                      ...
                      className={`p-2 rounded-lg transition-all ${themeMuted} hover:text-pink-500 hover:bg-black/5`}
                      title="Edit content"
                    >
                      <Edit3 size={18} />
                    </button>
